...
(() => {
 ...
 function hintText(){
  if(!ready)return '接続が戻るまで操作をお待ちください。';
  if(state.pending)return me===state.presenter?'下の回答を1つ選んでください。':demo?'ひとり体験では、回答役に切り替えて答えられます。':'回答が届くと、そのまま履歴に残ります。';
  return me===state.presenter?(demo?'自動では進みません。下のボタンで続けられます。':'質問が届くと、回答ボタンが出ます。'):'質問ボタンを押してから、Discordで話そう。';
 }
 ...
 rail=()=>originalRail().replace('assets/grove.svg',GROVE).replace('FRIENDS PREVIEW · 0.3','FRIENDS PREVIEW · 0.4');

 header=()=>{
  const lobby=!state||state.phase==='lobby';
  const title=state?esc(state.scope):'いつもの通話に、ひとつの謎を。';
  const sub=lobby?'人が出題、みんなで推理。音声はDiscordのまま。':`出題：${esc(who(state.presenter))}　／　${state.mode==='sealed'?'FinePlayは終了後に発表':'FinePlayはその場で'}`;
 ...
 function tutorialBar(){
  const {i,tip}=tutorialState(),steps=STEPS[tutorial];
  return `<section class="fp-tutorial"><div class="fp-tutorial-head"><span class="fp-face sm is-turn"><img src="${FACE}" alt=""></span><b>${tutorial==='genie'?'魔人役（答える人）のお試し':'質問役（当てにいく人）のお試し'}</b><a class="fp-tutorial-swap" href="?demo=1&amp;role=${tutorial==='genie'?'asker':'genie'}">${tutorial==='genie'?'質問役を試す →':'魔人役を試す →'}</a></div><ol class="fp-steps">${steps.map((s,n)=>`<li class="${n<i?'done':n===i?'now':''}">${esc(s)}</li>`).join('')}</ol><p class="fp-tip">${esc(tip)}</p><p class="fp-tutorial-note">相手の質問・回答は台本のサンプルです。AIが自由文を読んで答えているわけではありません。</p></section>`;
 }
 function introModal(){
  const genie=tutorial==='genie';
  modal(`<h2>${genie?'あなたは「まじん係」。答えを知っている人です。':'あなたは「質問係」。答えを当てにいく人です。'}</h2>`+(genie
   ?'<p>この体験のお題は <b>傘</b>。本番では、出題者が答えを自分の頭か手元に持ちます。<b>アプリに正解を先に入力する欄はありません。</b></p><p>質問が届いたら <b>5択</b>（はい／いいえ／たぶんそう／たぶん違う／わからない）で答えます。文章を書いて答える必要はありません。</p>'
   :'<p>質問メモを書いて送ります。声だけで聞いてもかまいませんが、<b>履歴に残したいときは短く書きます</b>。</p><p>答えるのは、まじん係（人間）。このお試しでは、自分で回答役に切り替えて答えを返せます。「これだ」と思ったら <b>答えがわかった</b> で宣言します。</p>')
   +'<p class="small">相手の質問・回答は台本のサンプルです。AIが自由文を読んで答えているわけではありません。</p><div class="row"><button id="modal-close" class="primary">はじめる</button></div>');
 }

 demoBar=()=>`${tutorial?tutorialBar():''}<section class="demo-bar compact-demo"><b>ひとり体験 <span>${tutorial?'この端末だけの操作体験です':'一人で両方の役を試せます'}</span></b><details class="demo-options"><summary>体験の切り替え</summary><div class="row"><button id="demo-answer">回答役</button><button id="demo-ask">質問役</button><button id="demo-result">結果を見る</button><button id="demo-reset">最初から</button></div></details></section>`;
 ...
 bind=()=>{
  ...
  if(!introShown){introShown=true;try{sessionStorage.setItem(INTRO_KEY,'1');}catch{}setTimeout(introModal,80);}
 };
 function fitHistory(){
 ...
 render=()=>{
  ...
  const footer=document.querySelector('footer');if(footer?.firstChild?.nodeType===3)footer.firstChild.textContent='FINEPLAY 0.4 ';
 };
 if(demo&&tutorial){
  try{
   if(tutorial==='asker'){
